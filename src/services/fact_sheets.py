...
from src.services.ai_discovery_data import (
    create_ai_discovery_data_if_not_exists,
    delete_ai_discovery_data,
)
...
def get_fact_sheet(fact_sheet_name: str) -> Optional[FactSheet]:
    with get_session() as session:
        fact_sheet = (
            session.query(FactSheet)
            .filter(FactSheet.fact_sheet_name == fact_sheet_name)
            .first()
        )
    return fact_sheet
...
def delete_fact_sheet_by_repository(repository_id: UUID) -> None:
    with get_session() as session:
        session.query(FactSheet).filter(
            FactSheet.repository_id == repository_id
        ).delete()
        session.commit()


def create_fact_sheet_from_sbs(
    self_built_component: SelfBuiltComponent, repository_id: UUID
) -> Optional[FactSheet]:
    ...
    try:
        ...
        logger.debug(
            f"Updated AI discovery data for fact sheet: {fact_sheet.fact_sheet_id}"
        )
    except Exception as e:
        logger.error(
            f"Error managing AI discovery data for fact sheet {fact_sheet.fact_sheet_id}: {e}"
        )
