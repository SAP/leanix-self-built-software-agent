...
from src.services.fact_sheets import (
    create_fact_sheet_from_sbs,
    delete_fact_sheet_by_repository,
)
...
def get_repository(full_name: str) -> Optional[Repository]:
    with get_session() as session:
        repository = (
            session.query(Repository).filter(Repository.full_name == full_name).first()
        )
        return repository
    ...
    try:
        ...
        if repos_state.self_built_software:
            for sbs_component in repos_state.self_built_software:
                create_fact_sheet_from_sbs(
                    self_built_component=sbs_component, repository_id=repository.id
                )
            logger.info(
                f"Created {len(repos_state.self_built_software)} fact sheets for repository {repository.id}"
            )
        else:
            logger.info(
                f"No self-built software components found for repository {repository.id}"
            )

    except Exception as e:
        logger.error(
            f"Error processing fact sheets for repository {repository.id}: {e}"
        )
