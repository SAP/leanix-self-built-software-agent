...
def get_ai_discovery_data(fact_sheet_id: UUID) -> Optional[AiDiscoveryData]:
    with get_session() as session:
        return _build_filter_conditions(session, fact_sheet_id).first()


def delete_ai_discovery_data(fact_sheet_id: UUID) -> None:
    with get_session() as session:
        _build_filter_conditions(session, fact_sheet_id).delete()
        session.commit()


def create_ai_discovery_data_if_not_exists(
    fact_sheet_id: UUID, self_built_component: SelfBuiltComponent
) -> AiDiscoveryData:
    ...
    if data is None:
        with get_session() as session:
            ...
            if self_built_component.owner.individuals:
                contributors_dict = [
                    {"name": individual.name, "emails": individual.emails}
                    for individual in self_built_component.owner.individuals
                ]
            tech_stacks_dict = None
            if self_built_component.tech_stacks:
                tech_stacks_dict = [
                    dataclasses.asdict(stack)
                    for stack in self_built_component.tech_stacks
                ]
            data = AiDiscoveryData(
                fact_sheet_id=fact_sheet_id,
                languages=self_built_component.language,
                teams=self_built_component.owner.team
                if self_built_component.owner.team
                else None,
