...
from src.utils.file_filters import (
    ...
    _is_binary_file,
    _is_generated_or_derived_directory,
)
...
def sbs_name_discovery_runnable(state: RootRepoState) -> RootRepoState:
    ...
    discovered_services = discover_services_by_deployment_signals(
        local_repo_path, state.repo_root_url, state.deployable_signal_files
    )
...
def discover_services_by_deployment_signals(
    repo_path: Path, repo_root_url: str, deployment_signal_files: List[str]
) -> List[SelfBuiltComponent]:
    """
    ...
    referenced_dirs = _analyze_cicd_references(
        repo_path, deployment_signal_files, package_manager_dirs
    )
    logger.info(
        f"CI/CD files reference {len(referenced_dirs)} package manager directories"
    )
    ...
    if len(referenced_dirs) >= 4:
        for dir_info in package_manager_dirs:
            ...
            if str(relative_path) in referenced_dirs:
                service_name = _extract_service_name(
                    repo_path / relative_path, dir_info["package_file"]
                )
                language = dir_info["language"]

                # Group deployment signals for this service
                service_signals = _group_deployment_signals_for_service(
                    relative_path, deployment_signal_files
                )

                display_url = (
                    f"{repo_root_url}/tree/main/{relative_path}"
                ...
                logger.info(
                    f"Found service: {service_name} at {relative_path} (language: {language})"
                )
    else:
        ...
        for svc in discovered:
            ...
            name = svc.get("name") or (
                Path(path).name if path and path != "." else "root"
            )
            language = svc.get("language", "unknown")
            display_url = (
                f"{repo_root_url}/tree/main/{path}" if path != "." else repo_root_url
            )
            ...
            services.append(component)
            logger.info(
                f"(LLM) Detected service: {name} at {path} (language: {language})"
            )
...
def _find_package_manager_directories(repo_path: Path) -> List[Dict]:
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        filtered_files = [f for f in files if not _is_binary_file(f)]
...
def _analyze_cicd_references(
    repo_path: Path,
    deployment_signal_files: List[str],
    package_manager_dirs: List[Dict],
) -> Set[str]:
    """
    ...
    cicd_files = [f for f in deployment_signal_files if _is_cicd_file(f)]

    logger.info(
        f"Analyzing {len(cicd_files)} CI/CD files for service references: {cicd_files}"
    )
    ...
    for cicd_file in cicd_files:
        try:
            ...
            for dir_info in package_manager_dirs:
                ...
                logger.debug(
                    f"Checking if directory '{dir_path}' is referenced in CI/CD file '{cicd_file}'"
                )
                if _is_directory_referenced_in_cicd(content, dir_path, dir_info):
                    logger.info(
                        f"Directory '{dir_path}' referenced in CI/CD file '{cicd_file}'"
                    )
...
def _is_directory_referenced_in_cicd(
    content: str, dir_path: str, dir_info: Dict
) -> bool:
    """
    ...
    tool_patterns = _generate_tool_specific_patterns(
        service_name, dir_path, path_segments
    )
...
def _generate_service_name_patterns(
    service_name: str, path_segments: List[str]
) -> List[str]:
...
def _generate_tool_specific_patterns(
    service_name: str, dir_path: str, path_segments: List[str]
) -> List[str]:
    ...
    patterns.extend(
        [
            "services:",  # Look for service definitions (more complex matching needed)
...
def _group_deployment_signals_for_service(
    service_path: Path, deployment_signal_files: List[str]
) -> List[str]:
    ...
    for signal_file in deployment_signal_files:
        ...
        if (
            signal_file.startswith(service_path_str)
            or service_path_str == "."  # Root directory
            or _is_cicd_file(signal_file)
        ):  # CI/CD files can reference any service
...
def _extract_service_name(path: Path, package_file: str) -> str:
    ...
    logger.debug(
        f"Attempting to extract service name from path: {path}, package_file: {package_file}"
    )

    # If it's the root directory, try to extract from package.json or similar
    if service_name == "." or not service_name:
        logger.debug(
            "Path is root directory or has no name, attempting to extract from package file."
        )
        if package_file == "package.json":
            try:
                ...
                with open(package_json_path, "r") as f:
                    ...
                    logger.info(
                        f"Extracted service name from package.json: {extracted_name}"
                    )
