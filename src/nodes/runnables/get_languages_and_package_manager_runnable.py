...
def get_languages_and_package_manager_runnable(
    ...
    for expected_language in LANGUAGES:
        ...
        for file_extension in LANGUAGES[expected_language]["file_indicators"]:
            total_files += len(
                list(
                    glob.glob(
                        local_path + "/" + service_path + "/**/" + file_extension,
                        recursive=True,
                    )
                )
            )
        if total_files > 0:
            for expected_manifest_name in LANGUAGES[expected_language][
                "build_manifests"
            ]:
            ...
            languages.append(
                Language(
                    ...
                    packages_content=files_content,
                )
            )

    logger.info(
        f"Languages found {len(languages)} and manifest found {total_files_content_found} files for {service_name}"
    )

    # ── return structured result ───────────────────────────────
    return ServiceManifests(
        local_path=local_path, service_name=service_name, languages=languages
    )
