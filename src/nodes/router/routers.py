...
def route_on_repo_type(state: RootRepoState) -> str:
    ...
    logger.info("🔀 routing to %s based on repo_type=%s", nxt, state.repo_type)
    return nxt
...
def route_on_deployable(state: RootRepoState) -> str:
    ...
    if state.deployable:
        logger.info(
            f"Repository {state.repo_root_url} is deployable, proceeding to classify repo type"
        )
        return "classify_repo_type_runnable"
    else:
        logger.info(
            f"Repository {state.repo_root_url} is not deployable, ending workflow"
        )
