...
def _should_skip_directory(dir_name: str) -> bool:
    ...
    temp_dirs = {"tmp", "temp", "cache", ".cache", "logs", "log"}
...
def _is_binary_file(filename: str) -> bool:
    ...
    data_extensions = {".db", ".sqlite", ".sqlite3", ".mdb", ".accdb", ".dbf"}

    # Image and design files
    design_extensions = {".psd", ".ai", ".sketch", ".fig", ".xd"}
...
def _is_generated_or_derived_directory(dir_path: Path) -> bool:
    ...
    for pattern in generated_word_patterns:
        ...
        if (
            dir_name == pattern
            or dir_name.startswith(f"{pattern}-")
            or dir_name.endswith(f"-{pattern}")
            or f"-{pattern}-" in dir_name
        ):
            return True
